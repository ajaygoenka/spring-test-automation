...
@Component
@Scope("cucumber-glue")
public class FamilyPlanApi extends AbstractApiObject {
    private static final Logger log = (Logger) LoggerFactory.getLogger(FamilyPlanApi.class);
    ...
    public void setBaseUrl(String endpoint, String intOrExt) {
        if (intOrExt.equalsIgnoreCase("external experian") && endpoint.contains("{familyMemberId}")){
            endpoint = endpoint.replace("{familyMemberId}", scenarioSession.getData("familyMemberId"));
            baseUrl = envSetup.getBaseEnvSetup().getBaseUrl() + endpoint;
        } else if (intOrExt.equalsIgnoreCase("internal experian") && endpoint.contains("{familyMemberId}")) {
            endpoint = endpoint.replace("{familyMemberId}", scenarioSession.getData("familyMemberId"));
            baseUrl = envSetup.getFamilyPlanUrl() + "/internal" + endpoint;
        ...
        API_VERSION_NUMBER = envSetup.getFamilyPlanApiVersion();
