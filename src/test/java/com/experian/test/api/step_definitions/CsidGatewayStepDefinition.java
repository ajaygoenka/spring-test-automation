...
import com.experian.test.api.CsidGatewayApi;
...
public class CsidGatewayStepDefinition {
    ...
    @Autowired
    private CsidGatewayApi csidGatewayApi;

    @Autowired
    private EnvSetup envSetup;

    @Given("^I create a new (internal experian|external experian) CsidGateway service request to the (.*) endpoint$")
    public void create_new_api_request(String intOrExt, String endpoint) throws Throwable {
        csidGatewayApi.setBaseUrl(endpoint, intOrExt);
        commonStepDefinition.setApiObject(csidGatewayApi);
    }
}
