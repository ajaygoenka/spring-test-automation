...
import com.experian.test.api.SalesforceApi;
...
public class SalesForceStepDefinition {
    private static final Logger log = (Logger) LoggerFactory.getLogger(SalesForceStepDefinition.class);
    ...
    public SalesForceStepDefinition() {
    }

    @Given("^I create a new (internal experian|external experian) salesForce service request to the (.*) endpoint$")
    public void create_new_api_request(String intOrExt, String endpoint) throws Throwable {
        salesforceApi.setBaseUrl(endpoint, intOrExt);
        commonStepDefinition.setApiObject(salesforceApi);
    }
