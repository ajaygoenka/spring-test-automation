...
public class AlertsApi extends AbstractApiObject {
    private static final Logger log = (Logger) LoggerFactory.getLogger(AlertsApi.class);
    ...
    public void setBaseUrl(String endpoint, String intOrExt) {
        if (intOrExt.equalsIgnoreCase("internal experian") && endpoint.contains("{subscriptionId}")){
            endpoint = endpoint.replace("{subscriptionId}", (scenarioSession.getData("subscriptionId")));
            baseUrl = envSetup.getAlertsUrl()+ "/internal" + endpoint;
        ...
        API_VERSION_NUMBER = envSetup.getAlertsApiVersion();
