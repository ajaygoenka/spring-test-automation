...
@Component
@Scope("cucumber-glue")
public class OffersApi extends AbstractApiObject {
    private static final Logger log = (Logger) LoggerFactory.getLogger(OffersApi.class);
    ...
    public void setBaseUrl(String endpoint, String intOrExt) {
        if (intOrExt.equalsIgnoreCase("internal experian") && endpoint.contains("{offers}")){
            endpoint = endpoint.replace("{offers}", scenarioSession.getData("offers"));
            baseUrl = envSetup.getOffersUrl() + "/internal" + endpoint;
        ...
        API_VERSION_NUMBER = envSetup.getOffersApiVersion();
