...
@Component
@Scope("cucumber-glue")
public class LeadCaptureApi extends AbstractApiObject {

    private static final Logger log = (Logger) LoggerFactory.getLogger(LeadCaptureApi.class);
    ...
    public void setBaseUrl(String endpoint, String intOrExt) {
        ...
        if (intOrExt.equalsIgnoreCase("internal experian")){
            baseUrl = envSetup.getLeadcaptureUrl() + "/internal" + endpoint;
        } else if(intOrExt.equalsIgnoreCase("external experian")){
            baseUrl = envSetup.getBaseEnvSetup().getBaseUrl() + endpoint;
        }

        API_VERSION_NUMBER = envSetup.getLeadcaptureApiVersion();
